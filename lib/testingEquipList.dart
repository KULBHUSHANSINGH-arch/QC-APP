...
// import 'package:newqcm/CapaReport.dart';
...
import 'package:newqcm/testing_equi__list_model.dart';
...
class testingEquipList extends StatefulWidget {
  testingEquipList();
...
class _DirectoryState extends State<testingEquipList> {
  ...
  Future<List<UserData>?> getData() async {
    ...
    final url = (site! + 'TestEquipmet/getTestEquipment');
    http.post(
      Uri.parse(url),
...
// <List<UserData>> List<UserData>
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            TestingEquipment()), // No 'const' needed here
                  );
                },
                child: Container(
                  decoration: BoxDecoration(
          ...
          child: Expanded(
            ...
            child: TextField(
              ...
              decoration: AppStyles.textFieldInputDecoration.copyWith(
                  hintText: "Search Testing Equipment",
      ...
      Padding(
          ...
          child: Row(
            ...
            children: [
              Text(
                  data.data!.length > 1
                      ? '${data.data!.length} Calibration Report'
                      : '${data.data!.length} Calibration Report',
            ...
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  if (SearchController.text.isEmpty) {
                    return Container(
                        child: _tile(
                            data.data![index].testEquipmentId ?? '',
                            data.data![index].date ?? '',
                            data.data![index].type ?? '',
                            data.data![index].equipmentName ?? '',
                            // data.data![index].createdOn ?? '',
                            data.data![index].person ?? '',
                            data.data![index].modelNo ?? '',
                            data.data![index].pdfUrl ?? ''));
                  } else if ((data.data![index].type ?? '')
                          .toLowerCase()
                          .contains((SearchController.text).toLowerCase()) ||
                      data.data![index].testingDetailId!
                          .toLowerCase()
                          .contains((SearchController.text).toLowerCase())) {
                    return Container(
                        ...
                        child: _tile(
                            ...
                            data.data![index].modelNo ?? '',
                            data.data![index].pdfUrl ?? ''));
                  } else if (data.data![index].equipmentName!
                      .toLowerCase()
                      .contains((SearchController.text).toLowerCase())) {
                    return Container(
                        margin: const EdgeInsets.only(top: 10.0),
                        child: _tile(
                            ...
                            data.data![index].modelNo ?? '',
                            data.data![index].pdfUrl ?? ''));
                  } else if (data.data![index].person!
                      .toLowerCase()
                      .contains((SearchController.text).toLowerCase())) {
                    return Container(
                        margin: const EdgeInsets.only(top: 10.0),
                        child: _tile(
                            ...
                            data.data![index].modelNo ?? '',
                            data.data![index].pdfUrl ?? ''));
                  } else if ((data.data![index].modelNo!)
                      .toLowerCase()
                      .contains((SearchController.text).toLowerCase())) {
                    return Container(
                        margin: const EdgeInsets.only(top: 10.0),
  ...
  Widget _tile(String id, String cratedOn, String type, String equipmentName,
      String person, String modelNo, String pdfUrl) {
    return InkWell(
      onTap: () {
        // Navigator.of(context).pushAndRemoveUntil(
        //     MaterialPageRoute(
                      ...
                      children: [
                        ...
                        Text(
                          DateFormat('dd-MM-yyyy')
                              .format(DateTime.parse(cratedOn)),
                          style: const TextStyle(
                            fontWeight: FontWeight.w600,
                            fontSize: 12,
                            fontFamily: appFontFamily,
